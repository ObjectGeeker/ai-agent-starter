...
public class AgentChatController {
    ...
    public BaseResponse<String> createSession(@RequestParam String userId, @RequestParam String agentId) {
        String sessionId = agentChatService.createSession(userId, agentId);
        return ResultUtil.success(sessionId);
    }

    @PostMapping("chat")
    @Operation(summary = "非流式对话")
    public BaseResponse<String> chat(@RequestBody AgentChatRequest agentChatRequest) {
    ...
    public SseEmitter streamChat(@RequestBody AgentChatRequest agentChatRequest) {
    ...
    private AgentChatCommandEntity convertToCommand(AgentChatRequest agentChatRequest) {
        return AgentChatCommandEntity.builder()
                .userId(agentChatRequest.getUserId())
